...
const App: React.FC = () => {
  ...
  const fullOutfitHints = [
    ...
    'A red power suit with a matching blazer and trousers',
  ...
  const getHints = () => {
    ...
    if (isUpperSelected && isLowerSelected) {
      return fullOutfitHints;
    }
    if (isUpperSelected) {
      return upperBodyHints;
    }
    if (isLowerSelected) {
      return lowerBodyHints;
    }
    return fullOutfitHints;
  };
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col p-2 sm:p-2 lg:p-2 lg:h-screen">
        ...
        <main className="w-full max-w-[90%] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:flex-grow lg:min-h-0">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 flex flex-col space-y-4 ">
            ...
            >
              {isLoading ? <Spinner /> : <SparklesIcon className="mr-3 w-6 h-6" />}
                  ...
                  <div className="aspect-[3/4] w-full bg-gray-200 rounded-lg overflow-hidden shadow-inner border">
                      {isLoading ? (
                          ...
                          <img src={generatedImage} alt="Generated outfit" className="object-cover w-full h-full" />
                           ...
                           <div className="flex items-center justify-center h-full text-gray-400 p-4">Your new outfit will appear here</div>
                      )}
                  </div>
              </div>
          </div>
